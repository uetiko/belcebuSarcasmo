 ...
 /**
  ...
  private function AddAnAddress($kind, $address, $name = '') {
    if (!preg_match('/^(to|cc|bcc|ReplyTo)$/', $kind)) {
        $this->logger->log(NULL, 'Invalid recipient array: ' . kind, "INFO");
      return false;
    }
    ...
    if (!self::ValidateAddress($address)) {
      ...
      $this->logger->log("", $this->Lang('invalid_address').': '.$address, "INFO");
      return false;
    }
    if ($kind != 'ReplyTo') {
 ...
 */
  public function SetFrom($address, $name = '',$auto=1) {
    ...
    if (!self::ValidateAddress($address)) {
      ...
      $this->logger->log(NULL, $this->Lang('invalid_address').': '.$address, "INFO");
      return false;
    }
    $this->From = $address;
    $this->FromName = $name;
  ...
  public function SmtpConnect() {
    ...
    $this->logger->log($this->smtp->Connected(), NULL, "INFO");
    ...
    try {
      while($index < count($hosts) && !$connection) {
        ...
        $index++;
        if (!$connection) {
            //$this->logger->log(var_dump($connection), NULL, "INFO");
          throw new phpmailerException($this->Lang('connect_host'));
        }
      }
    } catch (phpmailerException $e) {
      $this->smtp->Reset();
      $this->logger->log($e->getLine(), $e->getMessage(), "SEVERE");
      throw $e;
    }
    return true;
  }
